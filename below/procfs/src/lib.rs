...
impl ProcReader {
    ...
    fn read_file_to_string(&mut self, path: &Path) -> Result<&str> {
        ...
        std::str::from_utf8(&self.buffer[..size])
    ...
    fn read_pid_stat_from_path<P: AsRef<Path>>(&mut self, path: P) -> Result<PidStat> {
        ...
        let iter = line.split_whitespace().enumerate();
                ...
                1 => {
                    if let Some(c) = parse_item!(path, Some(item), char, line)? {
                        if let Some(state) = PidState::from_char(c) {
                            ...
                            return Err(Error::InvalidPidState(path.to_path_buf(), c));
                        }
                    }
                }
    ...
    fn read_pid_cmdline_from_path<P: AsRef<Path>>(
        ...
        // SAFETY: `is_done` is true, the thread will not do anything more.
        unsafe { Box::from_raw(result_ptr) }.take().unwrap()
    }
